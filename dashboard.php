...
$totalRejectedClaims = $rowRejectedClaims['total'];
    ...
    function getWeeklyData($conn, $table, $dateColumn) {
        $weeklyData = array_fill(0, 7, 0); // Inisialisasi array dengan 7 hari (0-6)

        $query = "SELECT DAYOFWEEK($dateColumn) AS day, COUNT(*) AS total 
                FROM $table 
                WHERE YEARWEEK($dateColumn, 1) = YEARWEEK(NOW(), 1) 
                GROUP BY DAYOFWEEK($dateColumn)";
        
        $result = $conn->query($query);
        while ($row = $result->fetch_assoc()) {
            $index = ($row['day'] - 1) % 7; // Sesuaikan indeks (Minggu = 1, Senin = 2, dst.)
    ...
    // Ambil 5 item terakhir dari tabel lost_items
    $queryRecentItems = "SELECT nama_barang, image, status, created_at FROM lost_items ORDER BY created_at DESC LIMIT 6";
    $resultRecentItems = $conn->query($queryRecentItems);

    $queryRecentClaims = "
    SELECT c.passenger_name, li.nama_barang, c.updated_at
    FROM claims c
    JOIN lost_items li ON c.lost_item_id = li.id
    ORDER BY c.updated_at DESC
    LIMIT 5
    ";
    $resultRecentClaims = $conn->query($queryRecentClaims);
...
<div class="container">
    ...
    <ul>
        <li>
            <a class="active" href="dashboard.php" style="display: flex; align-items: center; gap: 10px;">
                <img src="asset/dashboard.png" alt="dashboard" style="width: 20px; height: 20px; border-radius: 10%; margin-bottom: 5px;"> <!-- Gambar -->
                Dashboard
            </a>            
        </li>        
        <li>
...
<div class="main-content">
    <div class="header">
        ...
        <div class="overview-box">
            <h2>Total Found Catalog</h2>
            <p class="value"><?= $totalCatalog ?></p>
        ...
        <section class="over-coba">
        <div class="recent-claims">
            <h2>Recent Claim Approval</h2>
            <?php while ($claim = $resultRecentClaims->fetch_assoc()): ?>
                <div class="claim">
                <img alt="User Image" height="50" src="https://storage.googleapis.com/a1aa/image/58eFsRkOiiwqKSxMbc9e4V3B32mZPEGCGeMPRV813GX2cA4nA.jpg" width="50"/>
                 <div class="info">
                <h3><?= htmlspecialchars($claim['passenger_name']) ?></h3>
                <p><?= htmlspecialchars($claim['nama_barang']) ?></p>
                <small><?= date('d M Y, H:i', strtotime($claim['updated_at'])) ?></small>
                </div>
                </div>
        <?php endwhile; ?>
        </div>
            ...
            <div class="catalog">
            <h2>Catalog</h2>
            <?php while ($item = $resultRecentItems->fetch_assoc()): ?>
                <div class="item">
                    <img src="<?= htmlspecialchars($item['image']) ?>" alt="Item Image">
                    <div class="info">
                        <h3><?= htmlspecialchars($item['nama_barang']) ?></h3>
                        <p>Added on: <?= date('d M Y', strtotime($item['created_at'])) ?></p>
                    </div>
                    <div class="status <?= $item['status'] === 'claimed' ? 'claimed' : 'unclaimed' ?>">
                        <?= ucfirst($item['status']) ?>
                    </div>
                </div>
            <?php endwhile; ?>
